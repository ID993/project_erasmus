...
const isAdmin = async (req, res, next) => {
  try {
    const user = await Korisnik.findById(req.user.korisnik_id).populate(
      "uloga"
    );
...
router.get("/not-from", authenticateToken, async (req, res) => {
  try {
    ...
    const ustanovas = await Ustanova.find({
      drzava: { $ne: userDrzavaId },
    }).populate("drzava", "naziv");
...
router.post("/", authenticateToken, isAdmin, async (req, res) => {
  try {
    const { ime, adresa, kontakt, drzava, quotaStudents, quotaProfessors } =
      req.body;
...
router.get("/:id", authenticateToken, async (req, res) => {
  try {
    const institution = await Ustanova.findById(req.params.id).populate(
      "drzava",
      "naziv"
    );
    if (!institution) {
